...
public class Graph extends Application {
	...
	public GraphicsContext migration() {
		...
		for (int j = 0; j < game.getRanking().size(); j++) {
			...
			gc.fillOval(game.getListeDesStand().get(game.getRanking().get(j)).getCoordonnees().getLongitude(),
					game.getListeDesStand().get(game.getRanking().get(j)).getCoordonnees().getLatitude(), 100, 100);
		...
		public void run() {
			...
			while (true) {
				...
				if ((game.getHeureDepuisDebutJeu() % 12) == 0) {
					...
					//Platform.runLater(() -> carte.getChildren().add(canvas));
					//Platform.runLater(() -> panelJoueur.add(carte, 0, 0));
					TreeView<String> treeView = new TreeView<String>(infoPlayer());
