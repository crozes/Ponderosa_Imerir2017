...
public class Graph extends Application {
	...
	/*
	 * public Graph(TheGame partie){ this.game=partie; }
	 * 
	 * public void init(String[] args) { //game=partie;
	...
	public GraphicsContext migration() {
		...
		// game.getMapDeLaPopulation().genererPopulation(carte_x, 0, carte_y, 0,
		// nbPop, game.getMeteoDuJour(), Meteo.soir, game.getListeDesStand());
		GraphicsContext gc = canvas.getGraphicsContext2D();
		// population
		for (int i = 0; i < game.getMapDeLaPopulation().getPopulation().size(); i++) {
			gc.setFill(Color.rgb(0, 0, 0, 1));
			gc.fillOval(game.getMapDeLaPopulation().getPopulation().get(i).getLatitude(),
					game.getMapDeLaPopulation().getPopulation().get(i).getLongitude(), 5, 5);

		}
		// joueur
		for (int j = 0; j < game.getRanking().size(); j++) {
			gc.setFill(Color.rgb(100, 255, 100, 0.7));
			gc.fillOval(game.getListeDesStand().get(game.getRanking().get(j)).getCoordonnees().getLatitude(),
					game.getListeDesStand().get(game.getRanking().get(j)).getCoordonnees().getLongitude(), 100, 100);
		...
		for (int k = 0; k < game.getRanking().size(); k++) {
			for (int l = 1; l < game.getListeMapItemJoueur().get(game.getRanking().get(k)).size(); l++) {
				...
				// gc.fillOval(400,/***/250,
				// 50,50);//game.getListeDesStand().get(l).getInfluence(),
				// game.getListeDesStand().get(l).getInfluence())
				double latitude = Double.parseDouble(String
						.valueOf(game.getListeMapItemJoueur().get(game.getRanking().get(k)).get(l).getLatitude()));
				double longitude = Double.parseDouble(String
						.valueOf(game.getListeMapItemJoueur().get(game.getRanking().get(k)).get(l).getLongitude()));
				double influence = Double.parseDouble(String
						.valueOf(game.getListeMapItemJoueur().get(game.getRanking().get(k)).get(l).getInfluence()));
	...
	private TreeItem<String> infoPlayer() {
		...
		for (int i = 0; i < game.getRanking().size(); i++) {
			...
			posJoueur.getChildren().addAll(
					new TreeItem<String>("pos X : "
							+ game.getListeDesStand().get(game.getRanking().get(i)).getCoordonnees().getLatitude()),
					new TreeItem<String>("pos Y : "
							+ game.getListeDesStand().get(game.getRanking().get(i)).getCoordonnees().getLongitude()));
			...
			for (int k = 1; k < game.getListeMapItemJoueur().get(game.getRanking().get(i)).size(); k++) {
				...
				pub.getChildren().addAll(
						new TreeItem<String>("pos X : "
								+ game.getListeMapItemJoueur().get(game.getRanking().get(i)).get(k).getLatitude()),
						new TreeItem<String>("pos Y : "
								+ game.getListeMapItemJoueur().get(game.getRanking().get(i)).get(k).getLongitude()),
						new TreeItem<String>("influence : " + Double.parseDouble(String.valueOf(
								game.getListeMapItemJoueur().get(game.getRanking().get(i)).get(k).getInfluence()))));

				posPub.getChildren().addAll(pub);
			}

			infoJoueur.getChildren()
					.addAll(new TreeItem<String>("rank :" + (i + 1)),
							new TreeItem<String>(
									"budjet : " + game.getListePlayerInfo().get(game.getRanking().get(i)).getCash()
											+ "€"),
							new TreeItem<String>("vente : "
									+ game.getListePlayerInfo().get(game.getRanking().get(i)).getSales() + "€"),
							new TreeItem<String>("profit : "
									+ game.getListePlayerInfo().get(game.getRanking().get(i)).getProfit() + "€"));
			for (int k = 0; k < game.getListeDesDrinkInfo().get(game.getRanking().get(i)).size(); k++) {
				TreeItem<String> drinkInfo = new TreeItem<String>(
						game.getListeDesDrinkInfo().get(game.getRanking().get(i)).get(k).getName());
				drinkInfo.getChildren().addAll(
						new TreeItem<String>("prix : "
								+ game.getListeDesDrinkInfo().get(game.getRanking().get(i)).get(k).getPrice() + "€"), // affichage
						new TreeItem<String>("alchool : " + String.valueOf(
								game.getListeDesDrinkInfo().get(game.getRanking().get(i)).get(k).getIsHasAlcohol())),
						new TreeItem<String>("froid : " + String.valueOf(
								game.getListeDesDrinkInfo().get(game.getRanking().get(i)).get(k).getIsCold())));
	...
	public void start(Stage primaryStage) throws Exception {
		...
		topLabel.setStyle(
				"-fx-background-color: white; -fx-border-color: black;-fx-alignment:CENTER; -fx-font-size: 15pt;");
		...
		label2.setStyle(
				"-fx-background-color: white; -fx-border-color: black;-fx-alignment:CENTER; -fx-font-size: 20pt ;");
		...
		/**
		 * carte : http://poj.b3dgs.com/images/course_engine/map.png
		...
		///////////////// affichage carte et information
		///////////////// gaphique////////////////////
